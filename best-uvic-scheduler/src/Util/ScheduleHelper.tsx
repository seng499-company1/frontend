const ScheduleList = {
  fall: [
    {
      course: {
        code: "CSC111",
        pengRequired: {
          fall: true,
          spring: true,
          summer: true,
        },
        title: "Fundamentals of Programming with Engineering Applications",
        yearRequired: 1,
      },
      sections: [
        {
          capacity: 0,
          professor: {
            id: 302243805019271754015630741331797737474,
            name: "Peter F. Driessen",
          },
          timeSlots: [
            {
              dayOfWeek: "TUESDAY",
              timeRange: ["08:30", "09:20"],
            },
            {
              dayOfWeek: "WEDNESDAY",
              timeRange: ["08:30", "09:20"],
            },
            {
              dayOfWeek: "FRIDAY",
              timeRange: ["08:30", "09:20"],
            },
          ],
        },
      ],
    },
    {
      course: {
        code: "CSC115",
        pengRequired: {
          fall: true,
          spring: true,
          summer: true,
        },
        title: "Fundamentals of Programming II",
        yearRequired: 1,
      },
      sections: [
        {
          capacity: 0,
          professor: {
            id: 302243469012634531020575007111904362498,
            name: "Michael D. Adams",
          },
          timeSlots: [
            {
              dayOfWeek: "MONDAY",
              timeRange: ["08:30", "09:50"],
            },
            {
              dayOfWeek: "THURSDAY",
              timeRange: ["08:30", "09:50"],
            },
          ],
        },
      ],
    },
    {
      course: {
        code: "CSC225",
        pengRequired: {
          fall: true,
          spring: true,
          summer: true,
        },
        title: "Algorithms and Data Structures I",
        yearRequired: 2,
      },
      sections: [
        {
          capacity: 0,
          professor: {
            id: 302243810723699455042663048066962161666,
            name: "Fayez Gebali",
          },
          timeSlots: [
            {
              dayOfWeek: "TUESDAY",
              timeRange: ["08:30", "09:20"],
            },
            {
              dayOfWeek: "WEDNESDAY",
              timeRange: ["08:30", "09:20"],
            },
            {
              dayOfWeek: "FRIDAY",
              timeRange: ["08:30", "09:20"],
            },
          ],
        },
      ],
    },
    {
      course: {
        code: "CSC226",
        pengRequired: {
          fall: true,
          spring: true,
          summer: true,
        },
        title: "Algorithms and Data Structures II",
        yearRequired: 3,
      },
      sections: [
        {
          capacity: 0,
          professor: {
            id: 302243555212875346540174308887722328066,
            name: "Panajotis Agathoklis",
          },
          timeSlots: [
            {
              dayOfWeek: "TUESDAY",
              timeRange: ["08:30", "09:20"],
            },
            {
              dayOfWeek: "WEDNESDAY",
              timeRange: ["08:30", "09:20"],
            },
            {
              dayOfWeek: "FRIDAY",
              timeRange: ["08:30", "09:20"],
            },
          ],
        },
      ],
    },
    {
      course: {
        code: "CSC230",
        pengRequired: {
          fall: false,
          spring: true,
          summer: true,
        },
        title: "Introduction to Computer Architecture",
        yearRequired: 2,
      },
      sections: [
        {
          capacity: 0,
          professor: {
            id: 302243742983620505346654405586884624386,
            name: "Riham AlTawny",
          },
          timeSlots: [
            {
              dayOfWeek: "MONDAY",
              timeRange: ["08:30", "09:50"],
            },
            {
              dayOfWeek: "THURSDAY",
              timeRange: ["08:30", "09:50"],
            },
          ],
        },
      ],
    },
    {
      course: {
        code: "SENG265",
        pengRequired: {
          fall: false,
          spring: true,
          summer: true,
        },
        title: "Software Development Methods",
        yearRequired: 2,
      },
      sections: [
        {
          capacity: 0,
          professor: {
            id: 302243742983620505346654405586884624386,
            name: "Riham AlTawny",
          },
          timeSlots: [
            {
              dayOfWeek: "TUESDAY",
              timeRange: ["09:30", "10:20"],
            },
            {
              dayOfWeek: "WEDNESDAY",
              timeRange: ["09:30", "10:20"],
            },
            {
              dayOfWeek: "FRIDAY",
              timeRange: ["09:30", "10:20"],
            },
          ],
        },
      ],
    },
    {
      course: {
        code: "CSC320",
        pengRequired: {
          fall: true,
          spring: true,
          summer: true,
        },
        title: "Foundations of Computer Science",
        yearRequired: 3,
      },
      sections: [
        {
          capacity: 0,
          professor: {
            id: 302243753520966119743811305528230019074,
            name: "Amirali Baniasadi",
          },
          timeSlots: [
            {
              dayOfWeek: "MONDAY",
              timeRange: ["08:30", "09:50"],
            },
            {
              dayOfWeek: "THURSDAY",
              timeRange: ["08:30", "09:50"],
            },
          ],
        },
      ],
    },
    {
      course: {
        code: "CSC360",
        pengRequired: {
          fall: false,
          spring: true,
          summer: true,
        },
        title: "Operating Systems",
        yearRequired: 3,
      },
      sections: [
        {
          capacity: 0,
          professor: {
            id: 302243860637441839029195731999650873346,
            name: "Chris Papadopoulos",
          },
          timeSlots: [
            {
              dayOfWeek: "TUESDAY",
              timeRange: ["09:30", "10:20"],
            },
            {
              dayOfWeek: "WEDNESDAY",
              timeRange: ["09:30", "10:20"],
            },
            {
              dayOfWeek: "FRIDAY",
              timeRange: ["09:30", "10:20"],
            },
          ],
        },
      ],
    },
    {
      course: {
        code: "CSC361",
        pengRequired: {
          fall: true,
          spring: false,
          summer: true,
        },
        title: "Computer Communications and Networks",
        yearRequired: 3,
      },
      sections: [
        {
          capacity: 0,
          professor: {
            id: 302243469012634531020575007111904362498,
            name: "Michael D. Adams",
          },
          timeSlots: [
            {
              dayOfWeek: "MONDAY",
              timeRange: ["10:00", "11:20"],
            },
            {
              dayOfWeek: "THURSDAY",
              timeRange: ["10:00", "11:20"],
            },
          ],
        },
      ],
    },
    {
      course: {
        code: "CSC370",
        pengRequired: {
          fall: true,
          spring: true,
          summer: true,
        },
        title: "Database Systems",
        yearRequired: 3,
      },
      sections: [
        {
          capacity: 0,
          professor: {
            id: 302243753520966119743811305528230019074,
            name: "Amirali Baniasadi",
          },
          timeSlots: [
            {
              dayOfWeek: "TUESDAY",
              timeRange: ["10:30", "11:20"],
            },
            {
              dayOfWeek: "WEDNESDAY",
              timeRange: ["10:30", "11:20"],
            },
            {
              dayOfWeek: "FRIDAY",
              timeRange: ["10:30", "11:20"],
            },
          ],
        },
      ],
    },
    {
      course: {
        code: "SENG310",
        pengRequired: {
          fall: true,
          spring: true,
          summer: true,
        },
        title: "Human Computer Interaction",
        yearRequired: 2,
      },
      sections: [
        {
          capacity: 0,
          professor: {
            id: 302243760017675445913486988198833946626,
            name: "Ashoka K. S. Bhat",
          },
          timeSlots: [
            {
              dayOfWeek: "MONDAY",
              timeRange: ["10:00", "11:20"],
            },
            {
              dayOfWeek: "THURSDAY",
              timeRange: ["10:00", "11:20"],
            },
          ],
        },
      ],
    },
    {
      course: {
        code: "SENG321",
        pengRequired: {
          fall: false,
          spring: false,
          summer: false,
        },
        title: "Requirements Engineering",
        yearRequired: 3,
      },
      sections: [
        {
          capacity: 0,
          professor: {
            id: 302243555212875346540174308887722328066,
            name: "Panajotis Agathoklis",
          },
          timeSlots: [
            {
              dayOfWeek: "TUESDAY",
              timeRange: ["11:30", "12:20"],
            },
            {
              dayOfWeek: "WEDNESDAY",
              timeRange: ["11:30", "12:20"],
            },
            {
              dayOfWeek: "FRIDAY",
              timeRange: ["11:30", "12:20"],
            },
          ],
        },
      ],
    },
    {
      course: {
        code: "SENG350",
        pengRequired: {
          fall: true,
          spring: true,
          summer: true,
        },
        title: "Software Architecture and Design",
        yearRequired: 3,
      },
      sections: [
        {
          capacity: 0,
          professor: {
            id: 302243849941639899603510156871217577986,
            name: "Michael L. McGuire",
          },
          timeSlots: [
            {
              dayOfWeek: "MONDAY",
              timeRange: ["11:30", "12:50"],
            },
            {
              dayOfWeek: "THURSDAY",
              timeRange: ["11:30", "12:50"],
            },
          ],
        },
      ],
    },
    {
      course: {
        code: "SENG360",
        pengRequired: {
          fall: true,
          spring: true,
          summer: true,
        },
        title: "Security Engineering",
        yearRequired: 3,
      },
      sections: [
        {
          capacity: 0,
          professor: {
            id: 302243799394072215502862772190177263618,
            name: "Xiaodai Dong",
          },
          timeSlots: [
            {
              dayOfWeek: "TUESDAY",
              timeRange: ["12:30", "13:20"],
            },
            {
              dayOfWeek: "WEDNESDAY",
              timeRange: ["12:30", "13:20"],
            },
            {
              dayOfWeek: "FRIDAY",
              timeRange: ["12:30", "13:20"],
            },
          ],
        },
      ],
    },
    {
      course: {
        code: "ECE255",
        pengRequired: {
          fall: false,
          spring: false,
          summer: false,
        },
        title: "Introduction to Computer Architecture",
        yearRequired: 2,
      },
      sections: [
        {
          capacity: 0,
          professor: {
            id: 302243782914614412535880552733035593730,
            name: "David Capson",
          },
          timeSlots: [
            {
              dayOfWeek: "TUESDAY",
              timeRange: ["10:30", "11:20"],
            },
            {
              dayOfWeek: "WEDNESDAY",
              timeRange: ["10:30", "11:20"],
            },
            {
              dayOfWeek: "FRIDAY",
              timeRange: ["10:30", "11:20"],
            },
          ],
        },
      ],
    },
    {
      course: {
        code: "ECE260",
        pengRequired: {
          fall: false,
          spring: false,
          summer: false,
        },
        title: "Continuous-Time Signals and Systems",
        yearRequired: 2,
      },
      sections: [
        {
          capacity: 0,
          professor: {
            id: 302243765801331309454783632527542321154,
            name: "Jens Bornemann",
          },
          timeSlots: [
            {
              dayOfWeek: "TUESDAY",
              timeRange: ["11:30", "12:20"],
            },
            {
              dayOfWeek: "WEDNESDAY",
              timeRange: ["11:30", "12:20"],
            },
            {
              dayOfWeek: "FRIDAY",
              timeRange: ["11:30", "12:20"],
            },
          ],
        },
      ],
    },
    {
      course: {
        code: "ECE355",
        pengRequired: {
          fall: false,
          spring: false,
          summer: false,
        },
        title: "Microprocessor-Based Systems",
        yearRequired: 3,
      },
      sections: [
        {
          capacity: 0,
          professor: {
            id: 302243776893274061451790895623695368194,
            name: "Lin Cai",
          },
          timeSlots: [
            {
              dayOfWeek: "MONDAY",
              timeRange: ["13:00", "14:20"],
            },
            {
              dayOfWeek: "THURSDAY",
              timeRange: ["13:00", "14:20"],
            },
          ],
        },
      ],
    },
    {
      course: {
        code: "ECE360",
        pengRequired: {
          fall: false,
          spring: false,
          summer: false,
        },
        title: "Control Theory and Systems I",
        yearRequired: 3,
      },
      sections: [
        {
          capacity: 0,
          professor: {
            id: 302243799394072215502862772190177263618,
            name: "Xiaodai Dong",
          },
          timeSlots: [
            {
              dayOfWeek: "TUESDAY",
              timeRange: ["13:30", "14:20"],
            },
            {
              dayOfWeek: "WEDNESDAY",
              timeRange: ["13:30", "14:20"],
            },
            {
              dayOfWeek: "FRIDAY",
              timeRange: ["13:30", "14:20"],
            },
          ],
        },
      ],
    },
    {
      course: {
        code: "CHEM101",
        pengRequired: {
          fall: false,
          spring: false,
          summer: false,
        },
        title: "Fundamentals of Chemistry from Atoms to Materials",
        yearRequired: 2,
      },
      sections: [
        {
          capacity: 358,
          professor: {
            id: 0,
            name: "Genevieve Boice",
          },
          timeSlots: [
            {
              dayOfWeek: "MONDAY",
              timeRange: ["13:00", "14:20"],
            },
            {
              dayOfWeek: "THURSDAY",
              timeRange: ["13:00", "14:20"],
            },
          ],
        },
        {
          capacity: 358,
          professor: {
            id: 0,
            name: "Genevieve Boice",
          },
          timeSlots: [
            {
              dayOfWeek: "TUESDAY",
              timeRange: ["9:30", "10:20"],
            },
            {
              dayOfWeek: "WEDNESDAY",
              timeRange: ["9:30", "10:20"],
            },
            {
              dayOfWeek: "FRIDAY",
              timeRange: ["9:30", "10:20"],
            },
          ],
        },
        {
          capacity: 299,
          professor: {
            id: 0,
            name: "Genevieve Boice",
          },
          timeSlots: [
            {
              dayOfWeek: "TUESDAY",
              timeRange: ["12:30", "13:20"],
            },
            {
              dayOfWeek: "WEDNESDAY",
              timeRange: ["12:30", "13:20"],
            },
            {
              dayOfWeek: "FRIDAY",
              timeRange: ["12:30", "13:20"],
            },
          ],
        },
      ],
    },
    {
      course: {
        code: "ENGR110",
        pengRequired: {
          fall: false,
          spring: false,
          summer: false,
        },
        title: "Design and Communication I",
        yearRequired: 1,
      },
      sections: [
        {
          capacity: 150,
          professor: {
            id: 0,
            name: "Flavio Firmani",
          },
          timeSlots: [
            {
              dayOfWeek: "TUESDAY",
              timeRange: ["14:30", "15:20"],
            },
          ],
        },
        {
          capacity: 150,
          professor: {
            id: 0,
            name: "Flavio Firmani",
          },
          timeSlots: [
            {
              dayOfWeek: "TUESDAY",
              timeRange: ["15:30", "16:20"],
            },
          ],
        },
      ],
    },
    {
      course: {
        code: "ENGR130",
        pengRequired: {
          fall: false,
          spring: false,
          summer: false,
        },
        title: "Introduction to Professional Practice",
        yearRequired: 1,
      },
      sections: [
        {
          capacity: 260,
          professor: {
            id: 0,
            name: "Leo Spalteholz",
          },
          timeSlots: [
            {
              dayOfWeek: "MONDAY",
              timeRange: ["9:00", "9:50"],
            },
            {
              dayOfWeek: "THURSDAY",
              timeRange: ["9:00", "9:50"],
            },
          ],
        },
        {
          capacity: 130,
          professor: {
            id: 0,
            name: "Ash Senini",
          },
          timeSlots: [
            {
              dayOfWeek: "WEDNESDAY",
              timeRange: ["15:30", "16:20"],
            },
            {
              dayOfWeek: "FRIDAY",
              timeRange: ["15:30", "16:20"],
            },
          ],
        },
        {
          capacity: 130,
          professor: {
            id: 0,
            name: "Ash Senini",
          },
          timeSlots: [
            {
              dayOfWeek: "WEDNESDAY",
              timeRange: ["15:30", "16:20"],
            },
            {
              dayOfWeek: "FRIDAY",
              timeRange: ["15:30", "16:20"],
            },
          ],
        },
      ],
    },
    {
      course: {
        code: "MATH100",
        pengRequired: {
          fall: false,
          spring: false,
          summer: false,
        },
        title: "Calculus I",
        yearRequired: 1,
      },
      sections: [
        {
          capacity: 250,
          professor: {
            id: 0,
            name: "Christopher Bose",
          },
          timeSlots: [
            {
              dayOfWeek: "MONDAY",
              timeRange: ["11:30", "12:50"],
            },
            {
              dayOfWeek: "THURSDAY",
              timeRange: ["11:30", "12:50"],
            },
          ],
        },
        {
          capacity: 250,
          professor: {
            id: 0,
            name: "Ryan Budney",
          },
          timeSlots: [
            {
              dayOfWeek: "MONDAY",
              timeRange: ["15:30", "16:20"],
            },
            {
              dayOfWeek: "WEDNESDAY",
              timeRange: ["15:30", "16:20"],
            },
            {
              dayOfWeek: "THURSDAY",
              timeRange: ["15:30", "16:20"],
            },
          ],
        },
        {
          capacity: 250,
          professor: {
            id: 0,
            name: "Tom Thompson",
          },
          timeSlots: [
            {
              dayOfWeek: "MONDAY",
              timeRange: ["15:30", "16:20"],
            },
            {
              dayOfWeek: "WEDNESDAY",
              timeRange: ["15:30", "16:20"],
            },
            {
              dayOfWeek: "THURSDAY",
              timeRange: ["15:30", "16:20"],
            },
          ],
        },
      ],
    },
    {
      course: {
        code: "MATH109",
        pengRequired: {
          fall: false,
          spring: false,
          summer: false,
        },
        title: "Introduction to Calculus",
        yearRequired: 1,
      },
      sections: [
        {
          capacity: 250,
          professor: {
            id: 0,
            name: "TBD",
          },
          timeSlots: [
            {
              dayOfWeek: "TUESDAY",
              timeRange: ["8:30", "9:20"],
            },
            {
              dayOfWeek: "WEDNESDAY",
              timeRange: ["8:30", "9:20"],
            },
            {
              dayOfWeek: "FRIDAY",
              timeRange: ["8:30", "9:20"],
            },
          ],
        },
        {
          capacity: 250,
          professor: {
            id: 0,
            name: "TBD",
          },
          timeSlots: [
            {
              dayOfWeek: "TUESDAY",
              timeRange: ["12:30", "13:20"],
            },
            {
              dayOfWeek: "WEDNESDAY",
              timeRange: ["12:30", "13:20"],
            },
            {
              dayOfWeek: "FRIDAY",
              timeRange: ["12:30", "13:20"],
            },
          ],
        },
        {
          capacity: 250,
          professor: {
            id: 0,
            name: "TBD",
          },
          timeSlots: [
            {
              dayOfWeek: "TUESDAY",
              timeRange: ["13:30", "14:20"],
            },
            {
              dayOfWeek: "WEDNESDAY",
              timeRange: ["13:30", "14:20"],
            },
            {
              dayOfWeek: "FRIDAY",
              timeRange: ["13:30", "14:20"],
            },
          ],
        },
      ],
    },
    {
      course: {
        code: "MATH110",
        pengRequired: {
          fall: false,
          spring: false,
          summer: false,
        },
        title: "Matrix Algebra for Engineers",
        yearRequired: 1,
      },
      sections: [
        {
          capacity: 160,
          professor: {
            id: 0,
            name: "Christopher Eagle",
          },
          timeSlots: [
            {
              dayOfWeek: "TUESDAY",
              timeRange: ["12:30", "13:20"],
            },
            {
              dayOfWeek: "WEDNESDAY",
              timeRange: ["12:30", "13:20"],
            },
            {
              dayOfWeek: "FRIDAY",
              timeRange: ["12:30", "13:20"],
            },
          ],
        },
        {
          capacity: 151,
          professor: {
            id: 0,
            name: "Christopher Eagle",
          },
          timeSlots: [
            {
              dayOfWeek: "TUESDAY",
              timeRange: ["10:30", "11:20"],
            },
            {
              dayOfWeek: "WEDNESDAY",
              timeRange: ["10:30", "11:20"],
            },
            {
              dayOfWeek: "FRIDAY",
              timeRange: ["10:30", "11:20"],
            },
          ],
        },
        {
          capacity: 160,
          professor: {
            id: 0,
            name: "TBD",
          },
          timeSlots: [
            {
              dayOfWeek: "TUESDAY",
              timeRange: ["8:30", "9:20"],
            },
            {
              dayOfWeek: "WEDNESDAY",
              timeRange: ["8:30", "9:20"],
            },
            {
              dayOfWeek: "FRIDAY",
              timeRange: ["8:30", "9:20"],
            },
          ],
        },
      ],
    },
    {
      course: {
        code: "PHYS110",
        pengRequired: {
          fall: false,
          spring: false,
          summer: false,
        },
        title: "Introductory Physics I",
        yearRequired: 1,
      },
      sections: [
        {
          capacity: 148,
          professor: {
            id: 0,
            name: "Mark Laidlaw",
          },
          timeSlots: [
            {
              dayOfWeek: "TUESDAY",
              timeRange: ["10:30", "11:20"],
            },
            {
              dayOfWeek: "WEDNESDAY",
              timeRange: ["10:30", "11:20"],
            },
            {
              dayOfWeek: "FRIDAY",
              timeRange: ["10:30", "11:20"],
            },
          ],
        },
        {
          capacity: 180,
          professor: {
            id: 0,
            name: "Mark Laidlaw",
          },
          timeSlots: [
            {
              dayOfWeek: "TUESDAY",
              timeRange: ["11:30", "12:20"],
            },
            {
              dayOfWeek: "WEDNESDAY",
              timeRange: ["11:30", "12:20"],
            },
            {
              dayOfWeek: "FRIDAY",
              timeRange: ["11:30", "12:20"],
            },
          ],
        },
        {
          capacity: 340,
          professor: {
            id: 0,
            name: "Mark Laidlaw",
          },
          timeSlots: [
            {
              dayOfWeek: "TUESDAY",
              timeRange: ["13:30", "14:20"],
            },
            {
              dayOfWeek: "WEDNESDAY",
              timeRange: ["13:30", "14:20"],
            },
            {
              dayOfWeek: "FRIDAY",
              timeRange: ["13:30", "14:20"],
            },
          ],
        },
      ],
    },
    {
      course: {
        code: "STAT260",
        pengRequired: {
          fall: false,
          spring: false,
          summer: false,
        },
        title: "Introduction to Probability and Statistics I",
        yearRequired: 2,
      },
      sections: [
        {
          capacity: 150,
          professor: {
            id: 0,
            name: "TBD",
          },
          timeSlots: [
            {
              dayOfWeek: "MONDAY",
              timeRange: ["15:30", "16:20"],
            },
            {
              dayOfWeek: "WEDNESDAY",
              timeRange: ["15:30", "16:20"],
            },
            {
              dayOfWeek: "THURSDAY",
              timeRange: ["15:30", "16:20"],
            },
          ],
        },
        {
          capacity: 150,
          professor: {
            id: 0,
            name: "TBD",
          },
          timeSlots: [
            {
              dayOfWeek: "TUESDAY",
              timeRange: ["12:30", "13:20"],
            },
            {
              dayOfWeek: "WEDNESDAY",
              timeRange: ["12:30", "13:20"],
            },
            {
              dayOfWeek: "FRIDAY",
              timeRange: ["12:30", "13:20"],
            },
          ],
        },
      ],
    },
    {
      course: {
        code: "MATH122",
        pengRequired: {
          fall: false,
          spring: false,
          summer: false,
        },
        title: "Logic and Foundations",
        yearRequired: 2,
      },
      sections: [
        {
          capacity: 80,
          professor: {
            id: 0,
            name: "Natasha Morrison",
          },
          timeSlots: [
            {
              dayOfWeek: "MONDAY",
              timeRange: ["8:30", "9:50"],
            },
            {
              dayOfWeek: "THURSDAY",
              timeRange: ["8:30", "9:50"],
            },
          ],
        },
        {
          capacity: 80,
          professor: {
            id: 0,
            name: "Natasha Morrison",
          },
          timeSlots: [
            {
              dayOfWeek: "TUESDAY",
              timeRange: ["16:30", "17:50"],
            },
            {
              dayOfWeek: "THURSDAY",
              timeRange: ["16:30", "17:50"],
            },
          ],
        },
        {
          capacity: 80,
          professor: {
            id: 0,
            name: "Jon Noel",
          },
          timeSlots: [
            {
              dayOfWeek: "TUESDAY",
              timeRange: ["9:30", "10:20"],
            },
            {
              dayOfWeek: "WEDNESDAY",
              timeRange: ["9:30", "10:20"],
            },
            {
              dayOfWeek: "FRIDAY",
              timeRange: ["9:30", "10:20"],
            },
          ],
        },
        {
          capacity: 80,
          professor: {
            id: 0,
            name: "Jon Noel",
          },
          timeSlots: [
            {
              dayOfWeek: "TUESDAY",
              timeRange: ["11:30", "12:20"],
            },
            {
              dayOfWeek: "WEDNESDAY",
              timeRange: ["11:30", "12:20"],
            },
            {
              dayOfWeek: "FRIDAY",
              timeRange: ["11:30", "12:20"],
            },
          ],
        },
      ],
    },
  ],
  spring: [
    {
      course: {
        code: "CSC111",
        pengRequired: {
          fall: true,
          spring: true,
          summer: true,
        },
        title: "Fundamentals of Programming with Engineering Applications",
        yearRequired: 1,
      },
      sections: [
        {
          capacity: 0,
          professor: {
            id: 302243805019271754015630741331797737474,
            name: "Peter F. Driessen",
          },
          timeSlots: [
            {
              dayOfWeek: "TUESDAY",
              timeRange: ["08:30", "09:20"],
            },
            {
              dayOfWeek: "WEDNESDAY",
              timeRange: ["08:30", "09:20"],
            },
            {
              dayOfWeek: "FRIDAY",
              timeRange: ["08:30", "09:20"],
            },
          ],
        },
      ],
    },
    {
      course: {
        code: "CSC115",
        pengRequired: {
          fall: true,
          spring: true,
          summer: true,
        },
        title: "Fundamentals of Programming II",
        yearRequired: 1,
      },
      sections: [
        {
          capacity: 0,
          professor: {
            id: 302243771426530847967551601669162795010,
            name: "Alexandra Branzan Albu",
          },
          timeSlots: [
            {
              dayOfWeek: "MONDAY",
              timeRange: ["08:30", "09:50"],
            },
            {
              dayOfWeek: "THURSDAY",
              timeRange: ["08:30", "09:50"],
            },
          ],
        },
      ],
    },
    {
      course: {
        code: "CSC225",
        pengRequired: {
          fall: true,
          spring: true,
          summer: true,
        },
        title: "Algorithms and Data Structures I",
        yearRequired: 2,
      },
      sections: [
        {
          capacity: 0,
          professor: {
            id: 302243810723699455042663048066962161666,
            name: "Fayez Gebali",
          },
          timeSlots: [
            {
              dayOfWeek: "TUESDAY",
              timeRange: ["08:30", "09:20"],
            },
            {
              dayOfWeek: "WEDNESDAY",
              timeRange: ["08:30", "09:20"],
            },
            {
              dayOfWeek: "FRIDAY",
              timeRange: ["08:30", "09:20"],
            },
          ],
        },
      ],
    },
    {
      course: {
        code: "CSC226",
        pengRequired: {
          fall: true,
          spring: true,
          summer: true,
        },
        title: "Algorithms and Data Structures II",
        yearRequired: 3,
      },
      sections: [
        {
          capacity: 0,
          professor: {
            id: 302243555212875346540174308887722328066,
            name: "Panajotis Agathoklis",
          },
          timeSlots: [
            {
              dayOfWeek: "TUESDAY",
              timeRange: ["08:30", "09:20"],
            },
            {
              dayOfWeek: "WEDNESDAY",
              timeRange: ["08:30", "09:20"],
            },
            {
              dayOfWeek: "FRIDAY",
              timeRange: ["08:30", "09:20"],
            },
          ],
        },
      ],
    },
    {
      course: {
        code: "CSC230",
        pengRequired: {
          fall: false,
          spring: true,
          summer: true,
        },
        title: "Introduction to Computer Architecture",
        yearRequired: 2,
      },
      sections: [
        {
          capacity: 0,
          professor: {
            id: 302243742983620505346654405586884624386,
            name: "Riham AlTawny",
          },
          timeSlots: [
            {
              dayOfWeek: "MONDAY",
              timeRange: ["08:30", "09:50"],
            },
            {
              dayOfWeek: "THURSDAY",
              timeRange: ["08:30", "09:50"],
            },
          ],
        },
      ],
    },
    {
      course: {
        code: "SENG265",
        pengRequired: {
          fall: false,
          spring: true,
          summer: true,
        },
        title: "Software Development Methods",
        yearRequired: 2,
      },
      sections: [
        {
          capacity: 0,
          professor: {
            id: 302243771426530847967551601669162795010,
            name: "Alexandra Branzan Albu",
          },
          timeSlots: [
            {
              dayOfWeek: "TUESDAY",
              timeRange: ["09:30", "10:20"],
            },
            {
              dayOfWeek: "WEDNESDAY",
              timeRange: ["09:30", "10:20"],
            },
            {
              dayOfWeek: "FRIDAY",
              timeRange: ["09:30", "10:20"],
            },
          ],
        },
      ],
    },
    {
      course: {
        code: "SENG275",
        pengRequired: {
          fall: true,
          spring: true,
          summer: false,
        },
        title: "Software Testing",
        yearRequired: 2,
      },
      sections: [
        {
          capacity: 0,
          professor: {
            id: 302243788539813951048648521874656067586,
            name: "Thomas E. Darcie",
          },
          timeSlots: [
            {
              dayOfWeek: "MONDAY",
              timeRange: ["10:00", "11:20"],
            },
            {
              dayOfWeek: "THURSDAY",
              timeRange: ["10:00", "11:20"],
            },
          ],
        },
      ],
    },
    {
      course: {
        code: "CSC320",
        pengRequired: {
          fall: true,
          spring: true,
          summer: true,
        },
        title: "Foundations of Computer Science",
        yearRequired: 3,
      },
      sections: [
        {
          capacity: 0,
          professor: {
            id: 302243765801331309454783632527542321154,
            name: "Jens Bornemann",
          },
          timeSlots: [
            {
              dayOfWeek: "MONDAY",
              timeRange: ["08:30", "09:50"],
            },
            {
              dayOfWeek: "THURSDAY",
              timeRange: ["08:30", "09:50"],
            },
          ],
        },
      ],
    },
    {
      course: {
        code: "CSC360",
        pengRequired: {
          fall: false,
          spring: true,
          summer: true,
        },
        title: "Operating Systems",
        yearRequired: 3,
      },
      sections: [
        {
          capacity: 0,
          professor: {
            id: 302243860637441839029195731999650873346,
            name: "Chris Papadopoulos",
          },
          timeSlots: [
            {
              dayOfWeek: "TUESDAY",
              timeRange: ["09:30", "10:20"],
            },
            {
              dayOfWeek: "WEDNESDAY",
              timeRange: ["09:30", "10:20"],
            },
            {
              dayOfWeek: "FRIDAY",
              timeRange: ["09:30", "10:20"],
            },
          ],
        },
      ],
    },
    {
      course: {
        code: "CSC361",
        pengRequired: {
          fall: true,
          spring: false,
          summer: true,
        },
        title: "Computer Communications and Networks",
        yearRequired: 3,
      },
      sections: [
        {
          capacity: 0,
          professor: {
            id: 302243469012634531020575007111904362498,
            name: "Michael D. Adams",
          },
          timeSlots: [
            {
              dayOfWeek: "MONDAY",
              timeRange: ["10:00", "11:20"],
            },
            {
              dayOfWeek: "THURSDAY",
              timeRange: ["10:00", "11:20"],
            },
          ],
        },
      ],
    },
    {
      course: {
        code: "CSC370",
        pengRequired: {
          fall: true,
          spring: true,
          summer: true,
        },
        title: "Database Systems",
        yearRequired: 3,
      },
      sections: [
        {
          capacity: 0,
          professor: {
            id: 302243753520966119743811305528230019074,
            name: "Amirali Baniasadi",
          },
          timeSlots: [
            {
              dayOfWeek: "TUESDAY",
              timeRange: ["10:30", "11:20"],
            },
            {
              dayOfWeek: "WEDNESDAY",
              timeRange: ["10:30", "11:20"],
            },
            {
              dayOfWeek: "FRIDAY",
              timeRange: ["10:30", "11:20"],
            },
          ],
        },
      ],
    },
    {
      course: {
        code: "SENG310",
        pengRequired: {
          fall: true,
          spring: true,
          summer: true,
        },
        title: "Human Computer Interaction",
        yearRequired: 2,
      },
      sections: [
        {
          capacity: 0,
          professor: {
            id: 302243760017675445913486988198833946626,
            name: "Ashoka K. S. Bhat",
          },
          timeSlots: [
            {
              dayOfWeek: "TUESDAY",
              timeRange: ["10:30", "11:20"],
            },
            {
              dayOfWeek: "WEDNESDAY",
              timeRange: ["10:30", "11:20"],
            },
            {
              dayOfWeek: "FRIDAY",
              timeRange: ["10:30", "11:20"],
            },
          ],
        },
      ],
    },
    {
      course: {
        code: "SENG321",
        pengRequired: {
          fall: false,
          spring: false,
          summer: false,
        },
        title: "Requirements Engineering",
        yearRequired: 3,
      },
      sections: [
        {
          capacity: 0,
          professor: {
            id: 302243555212875346540174308887722328066,
            name: "Panajotis Agathoklis",
          },
          timeSlots: [
            {
              dayOfWeek: "TUESDAY",
              timeRange: ["11:30", "12:20"],
            },
            {
              dayOfWeek: "WEDNESDAY",
              timeRange: ["11:30", "12:20"],
            },
            {
              dayOfWeek: "FRIDAY",
              timeRange: ["11:30", "12:20"],
            },
          ],
        },
      ],
    },
    {
      course: {
        code: "SENG371",
        pengRequired: {
          fall: false,
          spring: false,
          summer: false,
        },
        title: "Software Evolution",
        yearRequired: 3,
      },
      sections: [
        {
          capacity: 0,
          professor: {
            id: 302243469012634531020575007111904362498,
            name: "Michael D. Adams",
          },
          timeSlots: [
            {
              dayOfWeek: "MONDAY",
              timeRange: ["11:30", "12:50"],
            },
            {
              dayOfWeek: "THURSDAY",
              timeRange: ["11:30", "12:50"],
            },
          ],
        },
      ],
    },
    {
      course: {
        code: "ECE310",
        pengRequired: {
          fall: false,
          spring: false,
          summer: false,
        },
        title: "Digital Signal Processing I",
        yearRequired: 2,
      },
      sections: [
        {
          capacity: 0,
          professor: {
            id: 302243760017675445913486988198833946626,
            name: "Ashoka K. S. Bhat",
          },
          timeSlots: [
            {
              dayOfWeek: "TUESDAY",
              timeRange: ["11:30", "12:20"],
            },
            {
              dayOfWeek: "WEDNESDAY",
              timeRange: ["11:30", "12:20"],
            },
            {
              dayOfWeek: "FRIDAY",
              timeRange: ["11:30", "12:20"],
            },
          ],
        },
      ],
    },
    {
      course: {
        code: "ECE360",
        pengRequired: {
          fall: false,
          spring: false,
          summer: false,
        },
        title: "Control Theory and Systems I",
        yearRequired: 3,
      },
      sections: [
        {
          capacity: 0,
          professor: {
            id: 302243799394072215502862772190177263618,
            name: "Xiaodai Dong",
          },
          timeSlots: [
            {
              dayOfWeek: "TUESDAY",
              timeRange: ["12:30", "13:20"],
            },
            {
              dayOfWeek: "WEDNESDAY",
              timeRange: ["12:30", "13:20"],
            },
            {
              dayOfWeek: "FRIDAY",
              timeRange: ["12:30", "13:20"],
            },
          ],
        },
      ],
    },
    {
      course: {
        code: "ECE455",
        pengRequired: {
          fall: false,
          spring: false,
          summer: false,
        },
        title: "Real Time Computer Systems Design Project",
        yearRequired: 4,
      },
      sections: [
        {
          capacity: 0,
          professor: {
            id: 302243555212875346540174308887722328066,
            name: "Panajotis Agathoklis",
          },
          timeSlots: [
            {
              dayOfWeek: "MONDAY",
              timeRange: ["08:30", "09:50"],
            },
            {
              dayOfWeek: "THURSDAY",
              timeRange: ["08:30", "09:50"],
            },
          ],
        },
      ],
    },
    {
      course: {
        code: "ECE458",
        pengRequired: {
          fall: false,
          spring: false,
          summer: false,
        },
        title: "Communication Networks",
        yearRequired: 3,
      },
      sections: [
        {
          capacity: 0,
          professor: {
            id: 302243832749128634008148899072180355074,
            name: "Kin Fun Li",
          },
          timeSlots: [
            {
              dayOfWeek: "MONDAY",
              timeRange: ["13:00", "14:20"],
            },
            {
              dayOfWeek: "THURSDAY",
              timeRange: ["13:00", "14:20"],
            },
          ],
        },
      ],
    },
    {
      course: {
        code: "SENG401",
        pengRequired: {
          fall: true,
          spring: true,
          summer: true,
        },
        title: "Social and Professional Issues",
        yearRequired: 4,
      },
      sections: [
        {
          capacity: 0,
          professor: {
            id: 302243765801331309454783632527542321154,
            name: "Jens Bornemann",
          },
          timeSlots: [
            {
              dayOfWeek: "TUESDAY",
              timeRange: ["08:30", "09:20"],
            },
            {
              dayOfWeek: "WEDNESDAY",
              timeRange: ["08:30", "09:20"],
            },
            {
              dayOfWeek: "FRIDAY",
              timeRange: ["08:30", "09:20"],
            },
          ],
        },
      ],
    },
    {
      course: {
        code: "ENGR120",
        pengRequired: {
          fall: false,
          spring: false,
          summer: false,
        },
        title: "Design and Communication II",
        yearRequired: 1,
      },
      sections: [
        {
          capacity: 165,
          professor: {
            id: 0,
            name: "Ilamparithi Thirumarai Chelvan",
          },
          timeSlots: [
            {
              dayOfWeek: "TUESDAY",
              timeRange: ["14:30", "15:20"],
            },
          ],
        },
        {
          capacity: 165,
          professor: {
            id: 0,
            name: "Ilamparithi Thirumarai Chelvan",
          },
          timeSlots: [
            {
              dayOfWeek: "TUESDAY",
              timeRange: ["15:30", "16:20"],
            },
          ],
        },
      ],
    },
    {
      course: {
        code: "ENGR141",
        pengRequired: {
          fall: false,
          spring: false,
          summer: false,
        },
        title: "Engineering Mechanics",
        yearRequired: 1,
      },
      sections: [
        {
          capacity: 210,
          professor: {
            id: 0,
            name: "Flavio Firmani",
          },
          timeSlots: [
            {
              dayOfWeek: "MONDAY",
              timeRange: ["11:30", "12:50"],
            },
            {
              dayOfWeek: "THURSDAY",
              timeRange: ["11:30", "12:50"],
            },
          ],
        },
        {
          capacity: 210,
          professor: {
            id: 0,
            name: "Flavio Firmani",
          },
          timeSlots: [
            {
              dayOfWeek: "MONDAY",
              timeRange: ["15:30", "16:20"],
            },
            {
              dayOfWeek: "THURSDAY",
              timeRange: ["15:30", "16:20"],
            },
          ],
        },
      ],
    },
    {
      course: {
        code: "MATH101",
        pengRequired: {
          fall: false,
          spring: false,
          summer: false,
        },
        title: "Calculus II",
        yearRequired: 1,
      },
      sections: [
        {
          capacity: 215,
          professor: {
            id: 0,
            name: "Muhammad Awais",
          },
          timeSlots: [
            {
              dayOfWeek: "MONDAY",
              timeRange: ["8:30", "9:50"],
            },
            {
              dayOfWeek: "THURSDAY",
              timeRange: ["8:30", "9:50"],
            },
          ],
        },
        {
          capacity: 210,
          professor: {
            id: 0,
            name: "Torsten Schoeneberg",
          },
          timeSlots: [
            {
              dayOfWeek: "MONDAY",
              timeRange: ["14:30", "15:20"],
            },
            {
              dayOfWeek: "WEDNESDAY",
              timeRange: ["14:30", "15:20"],
            },
            {
              dayOfWeek: "THURSDAY",
              timeRange: ["14:30", "15:20"],
            },
          ],
        },
        {
          capacity: 210,
          professor: {
            id: 0,
            name: "Tom Thompson",
          },
          timeSlots: [
            {
              dayOfWeek: "TUESDAY",
              timeRange: ["8:30", "9:20"],
            },
            {
              dayOfWeek: "WEDNESDAY",
              timeRange: ["8:30", "9:20"],
            },
            {
              dayOfWeek: "FRIDAY",
              timeRange: ["8:30", "9:20"],
            },
          ],
        },
        {
          capacity: 210,
          professor: {
            id: 0,
            name: "Tom Thompson",
          },
          timeSlots: [
            {
              dayOfWeek: "TUESDAY",
              timeRange: ["12:30", "13:20"],
            },
            {
              dayOfWeek: "WEDNESDAY",
              timeRange: ["12:30", "13:20"],
            },
            {
              dayOfWeek: "FRIDAY",
              timeRange: ["12:30", "13:20"],
            },
          ],
        },
        {
          capacity: 210,
          professor: {
            id: 0,
            name: "Majid Mazrooei",
          },
          timeSlots: [
            {
              dayOfWeek: "TUESDAY",
              timeRange: ["13:30", "14:20"],
            },
            {
              dayOfWeek: "WEDNESDAY",
              timeRange: ["13:30", "14:20"],
            },
            {
              dayOfWeek: "FRIDAY",
              timeRange: ["13:30", "14:20"],
            },
          ],
        },
      ],
    },
    {
      course: {
        code: "PHYS111",
        pengRequired: {
          fall: false,
          spring: false,
          summer: false,
        },
        title: "Introductory Physics II",
        yearRequired: 1,
      },
      sections: [
        {
          capacity: 180,
          professor: {
            id: 0,
            name: "Mark Laidlaw",
          },
          timeSlots: [
            {
              dayOfWeek: "TUESDAY",
              timeRange: ["10:30", "11:20"],
            },
            {
              dayOfWeek: "WEDNESDAY",
              timeRange: ["10:30", "11:20"],
            },
            {
              dayOfWeek: "FRIDAY",
              timeRange: ["10:30", "11:20"],
            },
          ],
        },
        {
          capacity: 180,
          professor: {
            id: 0,
            name: "Bob Kowalewski",
          },
          timeSlots: [
            {
              dayOfWeek: "TUESDAY",
              timeRange: ["11:30", "12:20"],
            },
            {
              dayOfWeek: "WEDNESDAY",
              timeRange: ["11:30", "12:20"],
            },
            {
              dayOfWeek: "FRIDAY",
              timeRange: ["11:30", "12:20"],
            },
          ],
        },
        {
          capacity: 180,
          professor: {
            id: 0,
            name: "Mark Laidlaw",
          },
          timeSlots: [
            {
              dayOfWeek: "TUESDAY",
              timeRange: ["13:30", "14:20"],
            },
            {
              dayOfWeek: "WEDNESDAY",
              timeRange: ["13:30", "14:20"],
            },
            {
              dayOfWeek: "FRIDAY",
              timeRange: ["13:30", "14:20"],
            },
          ],
        },
      ],
    },
  ],
  summer: [
    {
      course: {
        code: "CSC115",
        pengRequired: {
          fall: true,
          spring: true,
          summer: true,
        },
        title: "Fundamentals of Programming II",
        yearRequired: 1,
      },
      sections: [
        {
          capacity: 0,
          professor: {
            id: 302243771426530847967551601669162795010,
            name: "Alexandra Branzan Albu",
          },
          timeSlots: [
            {
              dayOfWeek: "TUESDAY",
              timeRange: ["08:30", "09:20"],
            },
            {
              dayOfWeek: "WEDNESDAY",
              timeRange: ["08:30", "09:20"],
            },
            {
              dayOfWeek: "FRIDAY",
              timeRange: ["08:30", "09:20"],
            },
          ],
        },
      ],
    },
    {
      course: {
        code: "CSC225",
        pengRequired: {
          fall: true,
          spring: true,
          summer: true,
        },
        title: "Algorithms and Data Structures I",
        yearRequired: 2,
      },
      sections: [
        {
          capacity: 0,
          professor: {
            id: 302243810723699455042663048066962161666,
            name: "Fayez Gebali",
          },
          timeSlots: [
            {
              dayOfWeek: "TUESDAY",
              timeRange: ["08:30", "09:20"],
            },
            {
              dayOfWeek: "WEDNESDAY",
              timeRange: ["08:30", "09:20"],
            },
            {
              dayOfWeek: "FRIDAY",
              timeRange: ["08:30", "09:20"],
            },
          ],
        },
      ],
    },
    {
      course: {
        code: "CSC226",
        pengRequired: {
          fall: true,
          spring: true,
          summer: true,
        },
        title: "Algorithms and Data Structures II",
        yearRequired: 3,
      },
      sections: [
        {
          capacity: 0,
          professor: {
            id: 302243555212875346540174308887722328066,
            name: "Panajotis Agathoklis",
          },
          timeSlots: [
            {
              dayOfWeek: "TUESDAY",
              timeRange: ["08:30", "09:20"],
            },
            {
              dayOfWeek: "WEDNESDAY",
              timeRange: ["08:30", "09:20"],
            },
            {
              dayOfWeek: "FRIDAY",
              timeRange: ["08:30", "09:20"],
            },
          ],
        },
      ],
    },
    {
      course: {
        code: "CSC230",
        pengRequired: {
          fall: false,
          spring: true,
          summer: true,
        },
        title: "Introduction to Computer Architecture",
        yearRequired: 2,
      },
      sections: [
        {
          capacity: 0,
          professor: {
            id: 302243742983620505346654405586884624386,
            name: "Riham AlTawny",
          },
          timeSlots: [
            {
              dayOfWeek: "MONDAY",
              timeRange: ["08:30", "09:50"],
            },
            {
              dayOfWeek: "THURSDAY",
              timeRange: ["08:30", "09:50"],
            },
          ],
        },
      ],
    },
    {
      course: {
        code: "SENG265",
        pengRequired: {
          fall: false,
          spring: true,
          summer: true,
        },
        title: "Software Development Methods",
        yearRequired: 2,
      },
      sections: [
        {
          capacity: 0,
          professor: {
            id: 302243771426530847967551601669162795010,
            name: "Alexandra Branzan Albu",
          },
          timeSlots: [
            {
              dayOfWeek: "TUESDAY",
              timeRange: ["09:30", "10:20"],
            },
            {
              dayOfWeek: "WEDNESDAY",
              timeRange: ["09:30", "10:20"],
            },
            {
              dayOfWeek: "FRIDAY",
              timeRange: ["09:30", "10:20"],
            },
          ],
        },
      ],
    },
    {
      course: {
        code: "SENG275",
        pengRequired: {
          fall: true,
          spring: true,
          summer: false,
        },
        title: "Software Testing",
        yearRequired: 2,
      },
      sections: [
        {
          capacity: 0,
          professor: {
            id: 302243788539813951048648521874656067586,
            name: "Thomas E. Darcie",
          },
          timeSlots: [
            {
              dayOfWeek: "MONDAY",
              timeRange: ["10:00", "11:20"],
            },
            {
              dayOfWeek: "THURSDAY",
              timeRange: ["10:00", "11:20"],
            },
          ],
        },
      ],
    },
    {
      course: {
        code: "CSC320",
        pengRequired: {
          fall: true,
          spring: true,
          summer: true,
        },
        title: "Foundations of Computer Science",
        yearRequired: 3,
      },
      sections: [
        {
          capacity: 0,
          professor: {
            id: 302243765801331309454783632527542321154,
            name: "Jens Bornemann",
          },
          timeSlots: [
            {
              dayOfWeek: "MONDAY",
              timeRange: ["08:30", "09:50"],
            },
            {
              dayOfWeek: "THURSDAY",
              timeRange: ["08:30", "09:50"],
            },
          ],
        },
      ],
    },
    {
      course: {
        code: "CSC360",
        pengRequired: {
          fall: false,
          spring: true,
          summer: true,
        },
        title: "Operating Systems",
        yearRequired: 3,
      },
      sections: [
        {
          capacity: 0,
          professor: {
            id: 302243860637441839029195731999650873346,
            name: "Chris Papadopoulos",
          },
          timeSlots: [
            {
              dayOfWeek: "TUESDAY",
              timeRange: ["09:30", "10:20"],
            },
            {
              dayOfWeek: "WEDNESDAY",
              timeRange: ["09:30", "10:20"],
            },
            {
              dayOfWeek: "FRIDAY",
              timeRange: ["09:30", "10:20"],
            },
          ],
        },
      ],
    },
    {
      course: {
        code: "CSC370",
        pengRequired: {
          fall: true,
          spring: true,
          summer: true,
        },
        title: "Database Systems",
        yearRequired: 3,
      },
      sections: [
        {
          capacity: 0,
          professor: {
            id: 302243753520966119743811305528230019074,
            name: "Amirali Baniasadi",
          },
          timeSlots: [
            {
              dayOfWeek: "MONDAY",
              timeRange: ["10:00", "11:20"],
            },
            {
              dayOfWeek: "THURSDAY",
              timeRange: ["10:00", "11:20"],
            },
          ],
        },
      ],
    },
    {
      course: {
        code: "SENG310",
        pengRequired: {
          fall: true,
          spring: true,
          summer: true,
        },
        title: "Human Computer Interaction",
        yearRequired: 2,
      },
      sections: [
        {
          capacity: 0,
          professor: {
            id: 302243760017675445913486988198833946626,
            name: "Ashoka K. S. Bhat",
          },
          timeSlots: [
            {
              dayOfWeek: "TUESDAY",
              timeRange: ["10:30", "11:20"],
            },
            {
              dayOfWeek: "WEDNESDAY",
              timeRange: ["10:30", "11:20"],
            },
            {
              dayOfWeek: "FRIDAY",
              timeRange: ["10:30", "11:20"],
            },
          ],
        },
      ],
    },
    {
      course: {
        code: "ECE260",
        pengRequired: {
          fall: false,
          spring: false,
          summer: false,
        },
        title: "Continuous-Time Signals and Systems",
        yearRequired: 2,
      },
      sections: [
        {
          capacity: 0,
          professor: {
            id: 302243765801331309454783632527542321154,
            name: "Jens Bornemann",
          },
          timeSlots: [
            {
              dayOfWeek: "TUESDAY",
              timeRange: ["11:30", "12:20"],
            },
            {
              dayOfWeek: "WEDNESDAY",
              timeRange: ["11:30", "12:20"],
            },
            {
              dayOfWeek: "FRIDAY",
              timeRange: ["11:30", "12:20"],
            },
          ],
        },
      ],
    },
    {
      course: {
        code: "ECE310",
        pengRequired: {
          fall: false,
          spring: false,
          summer: false,
        },
        title: "Digital Signal Processing I",
        yearRequired: 2,
      },
      sections: [
        {
          capacity: 0,
          professor: {
            id: 302243776893274061451790895623695368194,
            name: "Lin Cai",
          },
          timeSlots: [
            {
              dayOfWeek: "MONDAY",
              timeRange: ["11:30", "12:50"],
            },
            {
              dayOfWeek: "THURSDAY",
              timeRange: ["11:30", "12:50"],
            },
          ],
        },
      ],
    },
    {
      course: {
        code: "SENG426",
        pengRequired: {
          fall: false,
          spring: false,
          summer: false,
        },
        title: "Software Quality Engineering",
        yearRequired: 4,
      },
      sections: [
        {
          capacity: 0,
          professor: {
            id: 302243742983620505346654405586884624386,
            name: "Riham AlTawny",
          },
          timeSlots: [
            {
              dayOfWeek: "TUESDAY",
              timeRange: ["08:30", "09:20"],
            },
            {
              dayOfWeek: "WEDNESDAY",
              timeRange: ["08:30", "09:20"],
            },
            {
              dayOfWeek: "FRIDAY",
              timeRange: ["08:30", "09:20"],
            },
          ],
        },
      ],
    },
    {
      course: {
        code: "SENG440",
        pengRequired: {
          fall: false,
          spring: false,
          summer: false,
        },
        title: "Embedded Systems",
        yearRequired: 4,
      },
      sections: [
        {
          capacity: 0,
          professor: {
            id: 302243782914614412535880552733035593730,
            name: "David Capson",
          },
          timeSlots: [
            {
              dayOfWeek: "MONDAY",
              timeRange: ["08:30", "09:50"],
            },
            {
              dayOfWeek: "THURSDAY",
              timeRange: ["08:30", "09:50"],
            },
          ],
        },
      ],
    },
    {
      course: {
        code: "SENG499",
        pengRequired: {
          fall: false,
          spring: false,
          summer: false,
        },
        title: "Design Project II",
        yearRequired: 4,
      },
      sections: [
        {
          capacity: 0,
          professor: {
            id: 302243771426530847967551601669162795010,
            name: "Alexandra Branzan Albu",
          },
          timeSlots: [
            {
              dayOfWeek: "MONDAY",
              timeRange: ["10:00", "11:20"],
            },
            {
              dayOfWeek: "THURSDAY",
              timeRange: ["10:00", "11:20"],
            },
          ],
        },
      ],
    },
    {
      course: {
        code: "ECON180",
        pengRequired: {
          fall: false,
          spring: false,
          summer: false,
        },
        title: "Introduction to Economics and Financial Project Evaluation",
        yearRequired: 2,
      },
      sections: [
        {
          capacity: 175,
          professor: {
            id: 0,
            name: "Moein Amini",
          },
          timeSlots: [
            {
              dayOfWeek: "MONDAY",
              timeRange: ["13:00", "14:20"],
            },
            {
              dayOfWeek: "THURSDAY",
              timeRange: ["13:00", "14:20"],
            },
          ],
        },
      ],
    },
  ],
};
const axios = require("axios");

export async function GetSchedule1() {
  try {
    const response = await axios.get(
      "https://uvic.immortalmind.ca:5000/schedules/company/1"
    );
    console.log("response  ", response);
    return response.data;
  } catch (error) {
    console.log(error);
    return [];
  }
}

export async function GetSchedule2() {
  try {
    const response = await axios.get(
      "https://uvic.immortalmind.ca:5000/schedules/company/2"
    );
    console.log("response  ", response);
    return response.data;
  } catch (error) {
    console.log(error);
    return [];
  }
}
